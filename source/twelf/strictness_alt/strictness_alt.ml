...
let rec find_strict_vars_pos tp g =
  let (s, dep, ann_pairs, c, tms, g') = find_strict_vars_pos_rec tp g in
  	let s_final = finalize s dep in
  		(Pos (ann_pairs, c, tms), (IdSet.inter g' s_final))


and find_strict_vars_neg tp g =
  let (s, dep, ann_pairs, c, tms, g') = find_strict_vars_neg_rec tp g in
  	let s_final = finalize s dep in
  		(Neg (ann_pairs, c, tms, (IdSet.diff s_final g')), (IdSet.inter g' s_final))
...
and find_strict_vars_pos_rec tp g =
  match tp with
    PiType (x, tpA, tpB) -> let (ann_tpA, sA) = find_strict_vars_neg tpA g in
                            let (s, dep, ann_pairs, tc, tms, g') = find_strict_vars_pos_rec tpB (IdSet.add x g)
                            in (s, (add_dep dep x (IdSet.inter sA g)), (x, ann_tpA)::ann_pairs, tc, tms, (IdSet.add x g'))
  | AppType (c, tms) -> ((union_fsvo_terms tms g), Hashtbl.create 16, [], c, tms, g)
  | ImpType (tpA, tpB) -> find_strict_vars_pos_rec tpB g                       
  | IdType t -> (IdSet.empty, Hashtbl.create 16, [], t, [], IdSet.empty)
...
and find_strict_vars_neg_rec tp g =
  ...
  | PiType (x, tpA, tpB) -> let (ann_tpA, sA) = find_strict_vars_pos tpA g in
                            let (s, dep, ann_pairs, tc, tms, g') = find_strict_vars_neg_rec tpB  (IdSet.add x g)
                            in (s, (add_dep dep x (IdSet.inter sA g)), (x, ann_tpA)::ann_pairs, tc, tms, (IdSet.add x g'))
  | AppType (c, tms) -> ((union_fsvo_terms tms g), Hashtbl.create 16, [], c, tms, g)
